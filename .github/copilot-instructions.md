...
**Firmware (`src/`)**: `GameMode` (abstract base, `src/game_mode/`) → `PlayerMode` (human v human) | `BotMode` (concrete, composes `EngineProvider*`). `Board` (`src/board/board.*`) is the public physical-board package root with a deliberately narrow API: lifecycle (`begin()` returns `bool`), LED settings (`getBrightness`/`setBrightness`/`getDimMultiplier`/`setDimMultiplier`/`saveLedSettings`), sensor cadence (`cadenceMs()`), `clearAllLayers()`, `BoardEffectHandle startConnectingStatus()` / `stopConnectingStatus(handle&)`, and accessors for its owned workflows. Internally it owns one `BoardRuntime` (`src/board/core/runtime.*`), the board-internal hardware/canvas/effects/IO boundary: it composes `BoardDriver`, `BoardCanvas` (7-layer pixel buffer with `uint64_t` presence masks: BACKGROUND, GAME, ASSISTANCE, FEEDBACK, MENU, EFFECT, OVERRIDE), `BoardInput` (pure occupancy/event ring fed by a FreeRTOS poll task on Core 1; access through `BoardRuntime::drainInputEvents()` / `copyInputOccupancy()` / `inputOccupied()`), `BoardEffects` (slot-based looping/transient animations addressed by `BoardEffectHandle{slot, generation}`, 6 slots, retained scratch canvas for full-layer composition), and `BoardRenderer` (FreeRTOS render task on Core 1, ~30 Hz, flushes the canvas to the LED strip). All canvas mutation must go through `BoardRuntime::lockCanvas()` which returns a `CanvasGuard` RAII wrapper holding `canvas` + `effects` references under the mutex. Long-lived workflows take `BoardRuntime&` and own their own visual helpers: `BoardGameplay` (physical chess interactions, holds `BoardFeedback` + `BoardAssistance`, drains `BoardInputEventBatch` values for player intent and resign gestures), `BoardDiagnostics` (sensor test), `BoardCalibration` (friend of `BoardRuntime` for direct driver access; private `load()`/`run()`/`save()` invoked at startup, public `trigger()` for runtime recalibration), and `BoardMenu` (instance-owned game-selection state machine + folded-in `confirmAction`/`confirmResume`). `MenuView` (`board/menus/view.h`) is the drawable menu primitive and snapshots input occupancy once per poll. Status animations are exposed as handles — `BoardGameplay::startThinkingStatus()` / `startWaitingStatus()` return `BoardEffectHandle`, cancelled via `stopStatusAnimation(handle&)`. `EngineProvider` (base class with FreeRTOS task lifecycle, `src/engine/`) → `StockfishProvider` (`src/engine/stockfish/`) / `LichessProvider` (`src/engine/lichess/`) / `LibreChessProvider` (`src/engine/librechess/`). `BotMode::update()` drives a non-blocking state machine (`BotState::PLAYER_TURN` / `BotState::ENGINE_THINKING`); engine requests are async via FreeRTOS tasks. External firmware may consume `Board`, its workflow accessors, `BoardEffectHandle`, and the `BoardGameSelection*` types from `board/gui/selection.h`, but must not include `BoardDriver`, `BoardRuntime`, `BoardCanvas`, `BoardEffects`, `BoardFeedback`, or `BoardAssistance` directly. Board internals are segmented under `src/board/core/` (runtime), `src/board/gui/` (visual helpers + layers), `src/board/menus/` (menu primitive), and `src/board/workflows/` (long-lived workflows). Each `GameMode` holds a `Game*` — no global chess state.
...
**Firmware** (`src/`): board public root (`src/board/board.*`, `config.*`); board runtime (`src/board/core/`: `runtime.*`, `driver.*`, `canvas.*`, `effects.*`, `input.*`, `renderer.*`, `colors.h`); board GUI helpers (`src/board/gui/`: `assistance.*`, `feedback.*`, `effect_animations.*`, `layers.h`, `selection.h`); board menu primitive (`src/board/menus/`: `view.*`); board workflows (`src/board/workflows/`: `gameplay.*`, `diagnostics.*`, `calibration.*`, `menu.*`); shared firmware utilities (`src/shared/serial_logger.*`, `utils.*`), `WiFiManagerESP32` (web server + API + WiFi + NVS — constructed with `Board*` and `LittleFSStorage*`), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`).
