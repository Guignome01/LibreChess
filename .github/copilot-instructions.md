...
**Firmware** (`src/`): `Board` facade (`src/board/board.*`), board primitives (`src/board/colors.h`, `state.*`), `BoardDriver` (`src/board/driver.*`, LED + sensors + calibration mapping), `BoardAnimationLifecycle` (`src/board/lifecycle.*`, animation queue/task/mutex lifecycle), `BoardAssistance` (`src/board/assistance.*`, setup/legal-move/castling/remote-move guidance), `BoardFeedback` (`src/board/feedback.*`, move-result/status/error visuals), `BoardCalibration` (`src/board/calibration.*`, serial-guided mapping + NVS persistence), board animations (`src/board/animations.*`, animation job definitions + visual execution through driver primitives), board menus (`src/board/menu.*`, `navigator.*`, `config.*`), `BoardDiagnostics` (`src/board/diagnostics.*`, sensor coverage diagnostics), `WiFiManagerESP32` (web server + API + WiFi + NVS), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`), `SerialLogger` (`ILogger` impl), `SystemUtils` (Arduino helpers).
