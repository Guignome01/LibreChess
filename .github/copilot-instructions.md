...
**Firmware (`src/`)**: `GameMode` (abstract base, `src/game_mode/`) → `PlayerMode` (human v human) | `BotMode` (concrete, composes `EngineProvider*`). `EngineProvider` (base class with FreeRTOS task lifecycle, `src/engine/`) → `StockfishProvider` (`src/engine/stockfish/`) / `LichessProvider` (`src/engine/lichess/`) / `LibreChessProvider` (`src/engine/librechess/`). `BotMode::update()` drives a non-blocking state machine (`BotState::PLAYER_TURN` / `BotState::ENGINE_THINKING`); engine requests are async via FreeRTOS tasks. The firmware-facing board dependency is `Board` (`src/board/board.*`), which privately composes `BoardSystem` (hardware/state/settings/scheduler service boundary) and `BoardGui` (internal visual coordinator for feedback, assistance, diagnostics, menus, stack flow, and `BoardLayering` composition); external firmware modules should not take `BoardDriver*` directly. `src/board/colors.h`, `selection.h`, `drawable.h`, `layering.h`, and `state.*` are board-local primitives. Each `GameMode` holds a `Game*` — no global chess state.
...
**Firmware** (`src/`): `Board` facade (`src/board/board.*`), `BoardGui` (`src/board/gui.*`, board-internal visual coordinator), `BoardLayering` (`src/board/layering.*`, persistent base + overlay LED composition), `BoardSystem` (`src/board/system.*`, board-internal service boundary composing `BoardDriver` + `BoardScheduler` + `BoardState`), board primitives (`src/board/colors.h`, `selection.h`, `drawable.h`, `layering.h`, `state.*`), `BoardDriver` (`src/board/driver.*`, LED + sensors + calibration mapping), `BoardScheduler` (`src/board/scheduler.*`, animation queue/task/mutex scheduler), `BoardAssistance` (`src/board/assistance.*`, setup/legal-move/castling/remote-move guidance), `BoardFeedback` (`src/board/feedback.*`, move-result/status/error visuals), `BoardCalibration` (`src/board/calibration.*`, serial-guided mapping + NVS persistence), board animations (`src/board/animations.*`, value-based animation job factories + visual execution through LED batch primitives), board menus (`src/board/menu.*`, `stack.*`, `config.*`), `BoardDiagnostics` (`src/board/diagnostics.*`, sensor coverage diagnostics), `WiFiManagerESP32` (web server + API + WiFi + NVS), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`), `SerialLogger` (`ILogger` impl), `SystemUtils` (Arduino helpers).
