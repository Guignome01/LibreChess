...
**Engine (`lib/engine/`)**: `search` (negamax + alpha-beta + quiescence, iterative deepening, check extensions, PVS, null move pruning, late move reductions, aspiration windows, root move reordering, transposition table, move ordering) and `Engine` (direct-call facade: `calculateMove(fen, limits) → SearchResult`). Depends on core only.
...
**Engine library** (`lib/engine/`): `search` (negamax + alpha-beta + quiescence, iterative deepening, check extensions, PVS, null move pruning, late move reductions, aspiration windows, root move reordering, transposition table, move ordering), `Engine` (direct-call facade over search: `calculateMove(fen, limits) → SearchResult`).
