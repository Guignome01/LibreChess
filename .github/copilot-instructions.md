...
**Dependency model**: `core ← game`. Game never imports engine internals directly.
...
### Key Components
**Core library** (`lib/core/`): `Position`, `movegen`, `eval`, `eval_params`, `attacks`, `piece`, `utils`, `bitboard`, `zobrist`, `fen`, `notation`, `epd`, `hash_table.h`, `types.h`, `move.h`, `logger.h`, `search`, `search_params`, `MovePicker` (`move_picker.h` — staged move generation + heuristic updates), `TranspositionTable` (in `search.h` — inherits `HashTableBase`), `Engine` (direct-call facade: `calculateMove(fen, limits) → SearchResult`), `stats.h` (`#ifdef STATS` counters). Per-file instruction files auto-load when editing each module (see `core.instructions.md` for the full listing).
