...
**Firmware (`src/`)**: `GameMode` (abstract base, `src/game_mode/`) → `PlayerMode` (human v human) | `BotMode` (concrete, composes `EngineProvider*`). `Board` (`src/board/board.*`) is the public physical-board package root with a deliberately narrow API: lifecycle (`begin()` returns `bool`), LED settings (`getBrightness`/`setBrightness`/`getDimMultiplier`/`setDimMultiplier`/`saveLedSettings`), sensor cadence (`cadenceMs()`), `clearAllSurfaces()`, `BoardAnimationHandle startConnectingStatus()` / `stopConnectingStatus(handle&)`, and accessors for its owned workflows. Internally it owns one `BoardRuntime` (`src/board/core/runtime.*`) plus one GUI-owned `BoardAnimations`. `BoardRuntime` is the board-internal hardware/canvas/scheduler/IO boundary: it composes `BoardDriver`, `BoardCanvas` (fixed-size ordered surface stack with `uint64_t` presence masks; newer active surfaces override older ones per square), `BoardInput` (pure occupancy/event ring with dropped-event count and max-depth diagnostics; fed by a cooperative FreeRTOS poll task on Core 1; access through `BoardRuntime::drainInputEvents()` / `copyInputOccupancy()` / `inputOccupied()`), `BoardScheduler` (generic 6-slot timed `BoardPainter` runner addressed by `BoardScheduledHandle{slot, generation}`; each scheduled painter owns one canvas surface), and `BoardRenderer` (FreeRTOS render task on Core 1, ~30 Hz, runs scheduled painters and flushes the canvas to the LED strip, bounded stop). `BoardAnimations` provides GUI-owned animation helpers and frame painters addressed by `BoardAnimationHandle` and is injected into workflows/helpers that need animation vocabulary. All canvas mutation must go through `BoardRuntime::lockCanvas()` which returns a `CanvasGuard` RAII wrapper holding the `canvas` reference under the mutex; animation scheduling/cancellation uses the injected `BoardAnimations&` while that guard is held. Long-lived workflows take `BoardRuntime&` plus `BoardAnimations&` when they need animations and own their own visual helpers: `BoardGameplay` (physical chess interactions, holds `BoardFeedback` + `BoardAssistance`, drains `BoardInputEventBatch` values for player intent and resign gestures), `BoardDiagnostics` (sensor test), `BoardCalibration` (friend of `BoardRuntime` for direct driver access; private `load()`/`run()`/`save()` invoked at startup, public `trigger()` for runtime recalibration), and `BoardMenu` (instance-owned game-selection state machine). Menu primitives live under `board/menus/`: `MenuPanel` owns shared physical-board draw/poll/debounce mechanics, `MenuSelection` wraps one selectable screen with an optional back button, `MenuPrompt` owns green/red modal prompts, and `options.h` defines option ids/layouts plus `BoardGameSelection*` result types. Status animations are exposed as handles — `BoardGameplay::startThinkingStatus()` / `startWaitingStatus()` return `BoardAnimationHandle`, cancelled via `stopStatusAnimation(handle&)`. `EngineProvider` (base class with FreeRTOS task lifecycle, `src/engine/`) → `StockfishProvider` (`src/engine/stockfish/`) / `LichessProvider` (`src/engine/lichess/`) / `LibreChessProvider` (`src/engine/librechess/`). `BotMode::update()` drives a non-blocking state machine (`BotState::PLAYER_TURN` / `BotState::ENGINE_THINKING`); engine requests are async via FreeRTOS tasks. External firmware may consume `Board`, its workflow accessors, `BoardAnimationHandle`, and the `BoardGameSelection*` types from `board/menus/options.h`, but must not include `BoardDriver`, `BoardRuntime`, `BoardCanvas`, `BoardScheduler`, `BoardAnimations`, `BoardFeedback`, or `BoardAssistance` directly. Board internals are segmented under `src/board/core/` (runtime/presentation primitives), `src/board/gui/` (visual helpers + animations), `src/board/menus/` (menu primitives + option/result types), and `src/board/workflows/` (long-lived workflows). Each `GameMode` holds a `Game*` — no global chess state.
...
**Firmware** (`src/`): board public root (`src/board/board.*`); board runtime (`src/board/core/`: `runtime.*`, `driver.*`, `canvas.*`, `scheduler.*`, `input.*`, `renderer.*`, `colors.h`); board GUI helpers (`src/board/gui/`: `assistance.*`, `feedback.*`, `animations.*`); board menu primitives (`src/board/menus/`: `options.h`, `panel.*`, `selection.*`, `prompt.*`); board workflows (`src/board/workflows/`: `gameplay.*`, `diagnostics.*`, `calibration.*`, `menu.*`); shared firmware utilities (`src/shared/serial_logger.*`, `utils.*`), `WiFiManagerESP32` (web server + API + WiFi + NVS — constructed with `Board*` and `LittleFSStorage*`), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`).
