...
**Firmware** (`src/`): board public root (`src/board/board.*`, `config.*`); board runtime (`src/board/core/`: `runtime.*`, `driver.*`, `canvas.*`, `painter.h`, `scheduler.*`, `input.*`, `renderer.*`, `colors.h`); board GUI helpers (`src/board/gui/`: `assistance.*`, `feedback.*`, `animations.*`); board menu primitives (`src/board/menus/`: `view.*`, `selection.h`); board workflows (`src/board/workflows/`: `gameplay.*`, `diagnostics.*`, `calibration.*`, `menu.*`); shared firmware utilities (`src/shared/serial_logger.*`, `utils.*`), `WiFiManagerESP32` (web server + API + WiFi + NVS — constructed with `Board*` and `LittleFSStorage*`), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`).
