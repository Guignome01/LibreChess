...
**Firmware (`src/`)**: `GameMode` (abstract base, `src/game_mode/`) → `PlayerMode` (human v human) | `BotMode` (concrete, composes `EngineProvider*`). `Board` (`src/board/board.*`) is the public physical-board package root with a deliberately narrow API: lifecycle, LED settings, sensor cadence, status helpers, and accessors for its owned workflows. Internally it owns one `BoardController` (`src/board/core/controller.*`), which is the board-internal hardware/settings/scheduler/runtime boundary: it owns `BoardDriver`, `BoardScheduler`, `BoardLayering`, `BoardFeedback`, `BoardAssistance`, and `BoardStack`. Board workflows inherit `BoardWorkflow` (`src/board/workflow.*`) to share that controller without depending on the public package root: `BoardGameplay` (physical chess interactions, holds a `BoardSnapshot<BOARD_ROWS, BOARD_COLS>`), `BoardDiagnostics` (sensor test), `BoardCalibration` (unified external trigger + startup calibration workflow; `BoardController` constructs it with `BoardDriver&` for private `load()`/`run()`/`save()` during startup), and `BoardMenu` (game-selection menus + folded-in `confirmAction`/`confirmResume`). Legacy board facades were removed; status visuals live on `Board`/`BoardController`. `MenuView` (`gui/menu.h`) is the drawable menu primitive used by `BoardMenu`. Visual modules take `BoardController&` + `BoardLayering&` as mandatory references; all persistent rendering routes through `BoardLayering`. `EngineProvider` (base class with FreeRTOS task lifecycle, `src/engine/`) → `StockfishProvider` (`src/engine/stockfish/`) / `LichessProvider` (`src/engine/lichess/`) / `LibreChessProvider` (`src/engine/librechess/`). `BotMode::update()` drives a non-blocking state machine (`BotState::PLAYER_TURN` / `BotState::ENGINE_THINKING`); engine requests are async via FreeRTOS tasks. External firmware may consume `Board`, its workflow accessors, and the `BoardGameSelection*` types from `gui/selection.h`, but must not include `BoardDriver`, `BoardController`, `BoardLayering`, `BoardFeedback`, or `BoardAssistance` directly. Board internals are segmented under `src/board/core/` and `src/board/gui/`, with board workflows/config at `src/board/`. Each `GameMode` holds a `Game*` — no global chess state.
...
**Firmware** (`src/`): board package root/workflows/config (`src/board/board.*`, `workflow.*`, `gameplay.*`, `diagnostics.*`, `calibration.*`, `menu.*`, `config.*`), board core runtime (`src/board/core/`: `controller.*`, `driver.*`, `colors.h`, `scheduler.*`, `snapshot.h`), board GUI modules (`src/board/gui/`: `assistance.*`, `feedback.*`, `menu.*` (the `MenuView` drawable), `stack.*`, `layering.*`, `animations.*`, `drawable.h`, `selection.h`), shared firmware utilities (`src/shared/serial_logger.*`, `utils.*`), `WiFiManagerESP32` (web server + API + WiFi + NVS — constructed with `Board*` and `LittleFSStorage*`), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`).
