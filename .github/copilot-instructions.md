...
**Firmware** (`src/`): `Board` facade (`src/board/board.*`), board-level workflows/config (`src/board/config.*`, `calibration.*`, `diagnostics.*`), board core runtime (`src/board/core/`: `system.*`, `driver.*`, `state.*`, `colors.h`, `layering.*`, `animations.*`, `scheduler.*`), board GUI/workflows (`src/board/gui/`: `gui.*`, `assistance.*`, `feedback.*`, `menu.*`, `stack.*`, `drawable.h`, `selection.h`), `WiFiManagerESP32` (web server + API + WiFi + NVS), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`), `SerialLogger` (`ILogger` impl), `SystemUtils` (Arduino helpers).
