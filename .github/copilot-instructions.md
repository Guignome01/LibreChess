...
### Key Components
**Core library** (`lib/core/`): `Position`, `movegen`, `eval`, `attacks`, `piece`, `utils`, `bitboard`, `zobrist`, `fen`, `notation`, `epd`, `trace`, `types.h`, `move.h`, `logger.h`. Per-file instruction files auto-load when editing each module (see `core.instructions.md` for the full listing).
...
**Engine library** (`lib/engine/`): `search` (fail-soft negamax + alpha-beta + quiescence, iterative deepening, extensive pruning/extensions/reductions) and `Engine` (direct-call facade: `calculateMove(fen, limits) → SearchResult`). Depends on core only. Per-file instruction files: `search.instructions.md`, `engine-facade.instructions.md`.
...
- **Documentation & Tests** — every code change must update affected docs and tests in the same operation, never deferred. When changing `lib/core/`, `lib/game/`, or `lib/engine/`: update the relevant per-file instruction file (e.g., `position.instructions.md`, `search.instructions.md`) and library-level instruction file (`core.instructions.md`, `game-library.instructions.md`, or `engine-library.instructions.md`), plus `architecture.md`, `project-structure.md`, `testing.instructions.md`, and unit tests as needed. See the Completion Checklist in each library instruction file and `docs/development/additional-topics.md` for full sync triggers.