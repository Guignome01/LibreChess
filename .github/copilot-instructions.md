...
**Firmware (`src/`)**: `GameMode` (abstract base, `src/game_mode/`) → `PlayerMode` (human v human) | `BotMode` (concrete, composes `EngineProvider*`). `Board` (`src/board/board.*`) is the public physical-board entry point with a deliberately narrow API (lifecycle + LED settings + sensor cadence). It privately composes `BoardSystem` (hardware/settings/scheduler service boundary) and `BoardGui` (thin coordinator owning `BoardLayering`, `BoardFeedback`, `BoardAssistance`, `BoardStack`). All board internals are reached through a single `BoardServices` facade exposed via `Board::services()`, which is restricted via a friend list to the board-internal workflow classes: `BoardGameplay` (physical chess interactions, holds an `OccupancySnapshot`), `BoardDiagnostics` (sensor test), `BoardCalibration` (external trigger; full workflow `BoardCalibrationWorkflow` constructed on demand by `BoardSystem::makeCalibrationWorkflow()`), `BoardMenu` (game-selection menus + folded-in `confirmAction`/`confirmResume`), and `BoardStatus` (clear LEDs + WiFi connecting animation). `MenuView` (`gui/menu.h`) is the drawable menu primitive used by `BoardMenu`. Visual modules take `BoardSystem&` + `BoardLayering&` as mandatory references; all persistent rendering routes through `BoardLayering`. `EngineProvider` (base class with FreeRTOS task lifecycle, `src/engine/`) → `StockfishProvider` (`src/engine/stockfish/`) / `LichessProvider` (`src/engine/lichess/`) / `LibreChessProvider` (`src/engine/librechess/`). `BotMode::update()` drives a non-blocking state machine (`BotState::PLAYER_TURN` / `BotState::ENGINE_THINKING`); engine requests are async via FreeRTOS tasks. External firmware may consume `Board`, the workflow classes above, and the `BoardGameSelection*` types from `gui/selection.h`, but must not include `BoardDriver`, `BoardSystem`, `BoardGui`, `BoardLayering`, `BoardServices`, `BoardFeedback`, or `BoardAssistance` directly. Board internals are segmented under `src/board/core/` and `src/board/gui/`, with board workflows/config at `src/board/`. Each `GameMode` holds a `Game*` — no global chess state.
...
**Firmware** (`src/`): board base/workflows/config (`src/board/board.*`, `gameplay.*`, `diagnostics.*`, `calibration.*`, `menu.*`, `status.*`, `services.*`, `gameplay_snapshot.h`, `config.*`), board core runtime (`src/board/core/`: `system.*`, `driver.*`, `colors.h`, `scheduler.*`), board GUI (`src/board/gui/`: `gui.*`, `assistance.*`, `feedback.*`, `menu.*` (the `MenuView` drawable), `stack.*`, `layering.*`, `animations.*`, `drawable.h`, `selection.h`), shared firmware utilities (`src/shared/serial_logger.*`, `utils.*`), `WiFiManagerESP32` (web server + API + WiFi + NVS — constructed with `Board*`, `BoardStatus*`, `BoardCalibration*`, `LittleFSStorage*`), `LittleFSStorage` (`IGameStorage` impl under `src/storage/`).
