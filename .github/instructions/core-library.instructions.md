...
description: "Core chess library classes — Game, Position, History, movegen, piece, utils, fen, notation, zobrist, types, interfaces. Use for any work on lib/core/, lib/game/, lib/engine/, game modes, or engine providers including bug fixes, feature additions, refactoring, or test writing."
---
...
| `attacks` | Precomputed leaper tables (`KNIGHT[64]`, `KING[64]`, `PAWN[2][64]`), O(1) slider functions (`rook` via first-rank table + Hyperbola Quintessence, `bishop` via HQ on diagonal masks, `queen` = rook+bishop), x-ray attack functions (`xrayRook`, `xrayBishop`), `between(s1, s2)` (strictly between, exclusive), `AttackInfo` struct + `computeAll(bb)` (per-piece-type and per-color attack maps; **pre-built** for king safety, mobility, and move ordering), `isSquareUnderAttack(bb, sq, color)` (per-piece-type attack table lookups), `see(bb, mailbox, move)` (Static Exchange Evaluation — swap algorithm with least-valuable-attacker iteration, delegates to `eval::materialValue()` for piece values with king sentinel of 20000) | Stateless namespace (~3 KiB tables, initialized once via `init()`) |
...
| `zobrist` | Zobrist key generation (constexpr xorshift64), piece-index mapping, full-board hash computation (`computeHash(bb, mailbox, turn, state, epLegal)` — EP legality passed in by caller), pawn-only hash (`computePawnHash`) for pawn hash table | Stateless namespace (header-only, `zobrist.cpp` only for `computeHash`) |
...
| 3 | Move-gen context | `PositionState.castlingRights`, `.epRow`, `.epCol` | Input to movegen for move generation and Zobrist hashing |
...
| 8 | Incremental accumulators | `int mgPST_`, `int egPST_`, `int material_` | Three white-relative accumulators tracked incrementally on every move. `mgPST_`/`egPST_` = material + PST score (MG/EG), `material_` = pure material balance. `make()` delegates to a shared private helper `updateAccumulators()` that applies capture/movement/castling-rook/promotion deltas. `unmake()` restores from `UndoInfo`; `reverseMove()` uses 1-deep `UndoCache` (hash-validated O(1) restore, falling back to full recompute on cache miss). Recomputed from scratch on `loadFEN()` and `initializeBoard()` via `eval::computeMaterialPST()`. Used by search for `evaluatePosition(bb, mg, eg)` overload and lazy evaluation. |
...
2. **Game** delegates to `Position::makeMove()`, which:
   ...
   - Builds a `Move` struct from coordinates, delegates board mutation to `make()`: updates `BitboardSet` + `mailbox_` in lockstep (piece movement, captures, castling rook, en passant removal, promotion), incrementally updates Zobrist hash via XOR deltas, incrementally updates material+PST accumulators (capture/movement/castling/promotion deltas), updates `kingSquare_[]` cache, caches `Move` + `UndoInfo` in 1-deep `UndoCache` for O(1) `reverseMove()`
   - Updates `PositionState` (castling rights, EP target, halfmove/fullmove clocks)
   - Records position hash in `HashHistory`
   - Runs `Position::isGameOver()` → sets `MoveResult.gameResult` if checkmate/stalemate/draw
...
- **Game is the only firmware entry point** — firmware (`src/`) must never include `Position`, `History`, `movegen`, `iterator`, `piece`, or `utils` directly. `Game` re-exports query wrappers (`getPossibleMoves`, `isDraw`, `forEachSquare`, `checkEnPassant`, `checkCastling`) and static utility wrappers (`isEmptySquare`, `pieceColor`, `pieceType`, `pieceToChar`, `colorName`, `squareName`, `fileChar`, `rankChar`). `checkEnPassant` and `checkCastling` are `Position` member methods (return `EnPassantInfo` / `CastlingInfo` from `types.h`) that delegate to `utils::checkEnPassant` / `utils::checkCastling` Square-based free functions; `Game` thin-wraps them. This guarantees recording, observer notification, and lifecycle tracking can't be bypassed. Native tests may include internal headers.
...
- **movegen and notation are stateless** — all context (board, turn, `PositionState`) is passed as parameters. This makes them safe to call from any context (temp boards for check detection, history replay, test assertions) without hidden state coupling. Game-end detection (check, checkmate, stalemate, draws) is implemented as `Position` static methods with the same stateless signature pattern.
...
- **Incremental Zobrist hashing** — `make()` XORs Zobrist key deltas inline (`hash_ ^= pieceKey[from] ^ pieceKey[to]` plus castling/EP/side-to-move changes). `computeHash(bb, mailbox, turn, state, epLegal)` remains for debug verification but is never called in the hot path. EP legality is pre-computed by the caller and passed in as `epLegal` (avoids `computeHash` depending on movegen). `recomputeDerived()` calls `computeHash()` for cold paths (FEN load, reverseMove cache-miss), computing `epLegal` via `movegen::hasLegalEnPassantCapture()`. `computePawnHash()` XORs piece keys for all white (index 0) and black (index 6) pawns — used as the lookup key for the pawn hash table.

- **Incremental PST/material** — `make()` delegates accumulator updates to the shared private helper `updateAccumulators(piece, from, to, captured, capturedSq, isCastling, promotedTo)`, which applies capture removal, piece movement, castling rook, and promotion deltas in one place.  `makeMove()` delegates to `make()` (single code path for both game and search).  `reverseMove()` uses a 1-deep `UndoCache` (private struct in `Position`) that stores `Move` + `UndoInfo` + post-move hash for validation.  If the current hash matches `postHash` (hot path: immediate undo), `reverseMove()` delegates to `unmake()` for O(1) restore; otherwise falls back to manual board reversal + `recomputeDerived()` (cold path: multi-undo).  This is a pragmatic ESP32-optimized variant of the CPW-recommended undo-stack pattern — O(1) for 97%+ of perft calls, graceful degradation for game-play undo.  Reference: [CPW — Copy-Make](https://www.chessprogramming.org/Copy-Make).

- **Flat PSQT lookup tables** — `eval` pre-builds `PSQT_MG[12][64]` and `PSQT_EG[12][64]` combining `MATERIAL[type] + PST[type][sq]` with color sign baked in (white positive, black negative + mirror).  `pieceSquareMGEG()` returns `PSQTPair{mg, eg}` as a single array lookup with no branches.  Tables are lazy-built on first use; TUNING builds invalidate via `invalidatePSQT()` after parameter changes.

- **Castling rights lookup table** — `updateCastlingRights()` uses a 64-entry `CASTLING_MASK[sq]` table instead of 8 if-statements.  A move from or to a special square (king start, rook corner) ANDs the current rights with the corresponding mask (0xFF for non-special squares = no-op).  This works without piece-type checks because a non-rook on a rook square implies the rook already moved (rights already 0).  Reference: [CPW — Castling Rights](https://www.chessprogramming.org/Castling_Rights).
...
- **Move ordering during search** — staged via `MovePicker`: TT move → good captures (MVV-LVA + captureHistory, SEE≥0) → killer moves (2 per ply) → countermove heuristic → history heuristic (quiet moves) → bad captures (SEE<0). Score arrays use `int16_t` to reduce per-ply stack footprint (~1.7 KiB saved per negamax ply). `pickBestInRange()` selection sort — O(N) per move, avoids full sort since alpha-beta prunes most branches.
...
- **Color-parameterized eval loops**: Evaluation subfunctions that compute symmetric terms for both colors use `for (int c = 0; c < 2; ++c)` with `int sign = (c == 0) ? 1 : -1` and `c * 6` offsets into `byPiece[]`. Applied across all bilateral eval terms: `evalPawnStructure`, `evalPassedPawnKingDist`, `evalRookOnSeventh`, `evalBishopPair`, `evalRookFiles`, `evalKnightOutposts`, `evalBadBishop`, `evalRookBehindPasser`, `evalShieldOneSide`, `evalMobility`, `evalThreats`, `evalKingDanger`, `evalTrappedPieces`. New eval functions must follow this pattern — never duplicate white/black code.
- **Shared movegen helper**: `addPieceMoves(attacks, sq, bb, color, moves)` extracts the common pattern for non-king piece move generation (filter friendly squares, tag captures, emit to `MoveList`). Used by `addRookMoves`, `addBishopMoves`, `addQueenMoves`, `addKnightMoves` — each is a one-liner calling the helper with piece-specific attack generation.
- **Castling bit mapping**: `castlingCharToBit()` is the single source of truth for K/Q/k/q → bitmask (switch-based). `hasCastlingRight()` wraps it with color+side via `BIT[2][2]` lookup table indexed by `[raw(color)][kingSide]`. All castling rights logic should use these.
...
- **Pin-aware move generation**: `movegen::getPossibleMoves`, `movegen::hasAnyLegalMove`, `generateAllMoves`, `generateCaptures`, and `generateQuiets` compute a `LegalityContext` (checker info + pin data + check mask) once per call. Non-king/non-EP moves are filtered via bitwise AND against `pinRayFor(pinData, sq) & checkMask` — no `leavesInCheck` call needed. Only king moves and EP captures still use copy-make (`leavesInCheck`). Three generation modes: (1) self-contained (`generateAllMoves`/`generateCaptures`) build their own context internally, (2) staged: `buildLegalityContext()` once, then `generateCaptures(ctx)` and `generateQuiets(ctx)` reusing the same context — avoids double pin/check computation in staged move pickers. `PinData` and `LegalityContext` are public structs in `movegen.h`. File-local helpers in `movegen.cpp`: `pinRayFor`, `computePinData`. `attacks::attackersOfSquare()` computes the full attacker bitboard for check detection using a base+offset indexing pattern (`base = color * 6`, offsets 0–5 for pawn through king) for concise per-piece-type lookups; `attacks::isSquareUnderAttack()` is a thin wrapper.
