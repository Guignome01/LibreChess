...
- `reverseMove(MoveEntry&)` — undo by replaying from an earlier snapshot
...
- **`makeNullMove()` does NOT advance `halfmoveClock`** — a null move is a hypothetical pass (side-to-move flip + Zobrist update only), not a real move.  Incrementing the 50-move counter would cause NMP / null-window searches to trigger a spurious 50-move draw score in long endgames.  Side-to-move, en-passant clearing, and Zobrist updates are still applied (matches the [CPW — Null Move Pruning](https://www.chessprogramming.org/Null_Move_Pruning) convention).
- **Halfmove clock saturation** — real quiet moves increment `PositionState::halfmoveClock` only up to 100, matching FEN validation/storage bounds and the 50-move rule. Pawn moves and captures still reset it to 0.
...
- **`recordPosition()` sliding window** — appends the current hash, and when the array is full, compacts by keeping only the last `halfmoveClock + 1` entries (the window reachable by `hasRepeated()`), capped at `HashHistory::MAX_SIZE - 1` so the append always has a slot. This prevents silent drops in long games that would break repetition detection and prevents fixed-buffer overflow at the array boundary. After compaction, `make()` and `makeNullMove()` detect the stale `UndoInfo.historyCount` (saved pre-compaction) and adjust it to `hashHistory_.count - 1`, ensuring `unmake()` restores a consistent count. `loadFEN()` still resets `count = 0` (no undo path). [CPW — Repetitions](https://www.chessprogramming.org/Repetitions)
