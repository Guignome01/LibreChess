...
// ---------------------------------------------------------------------------
// LibreChessEngine — on-board chess engine using Game::calculateMove().
//
...
LibreChessEngine::LibreChessEngine(LibreChess::Game* game, int level,
  ...
  : AsyncEngineProvider(logger), game_(game), playerColor_(playerColor) {
...
LibreChessEngine::~LibreChessEngine() {
  // Stop any running search task before teardown.
  cancelRequest();
}

bool LibreChessEngine::initialize(EngineInitResult& result) {
  logger_.info("LibreChessEngine: initializing on-board engine");
  ...
  if (!game_->searchInitialized()) {
    ...
    logger_.info("LibreChessEngine: reusing existing Game search resources");
  }
  if (!game_->searchInitialized()) {
  ...
  result.mode = static_cast<uint8_t>(GameModeId::BOT);
...
void LibreChessEngine::requestMove(const std::string& fen) {
  auto* ctx = new (std::nothrow) TaskContext();
  if (!ctx) {
    logger_.error("LibreChessEngine: failed to allocate task context");
...
int LibreChessEngine::getEvaluation() {
...
void LibreChessEngine::taskFunction(void* param) {
