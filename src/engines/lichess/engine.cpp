...
LichessEngine::LichessEngine(const LichessConfig& config, LibreChess::ILogger* logger)
  : AsyncEngineProvider(logger), config_(config), api_(config_, logger) {}

// ---------------------------------------------------------------
// initialize() — blocking game discovery (BotMode wraps with animation)
// ---------------------------------------------------------------

bool LichessEngine::initialize(EngineInitResult& result) {
  if (config_.apiToken.length() == 0) {
    logger_.error("LichessEngine: no API token configured");
    return false;
  }

  String username;
  if (!api_.verifyToken(username)) {
    logger_.error("LichessEngine: invalid API token");
    return false;
  }
  ...
  while (event.type != LichessEventType::GAME_START) {
    ...
    if (millis() > initDeadline) {
      logger_.error("LichessEngine: timed out waiting for game");
      return false;
    }
    delay(2000);
  }
  ...
  result.mode = static_cast<uint8_t>(GameModeId::LICHESS);
  result.difficulty = 0;
...
void LichessEngine::requestMove(const std::string& /*fen*/) {
  auto* ctx = new (std::nothrow) TaskContext();
  if (!ctx) {
    logger_.error("LichessEngine: failed to allocate task context");
...
bool LichessEngine::checkResult(EngineResult& result) {
...
bool LichessEngine::onPlayerMoveApplied(const std::string& moveCoord) {
  ...
  for (int attempt = 0; attempt < maxRetries; attempt++) {
    ...
    logger_.errorf("LichessEngine: send failed, attempt %d/%d", attempt + 1, maxRetries);
    delay(500);
  }

  logger_.error("LichessEngine: all send attempts failed");
  lastSentMove_ = "";
  return false;
}

void LichessEngine::onResignConfirmed() {
...
void LichessEngine::taskFunction(void* param) {
