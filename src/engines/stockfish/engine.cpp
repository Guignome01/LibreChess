...
#include "engines/stockfish/api.h"
...
StockfishEngine::StockfishEngine(int level, char playerColor, LibreChess::ILogger* logger)
  : AsyncEngineProvider(logger), playerColor_(playerColor) {
...
bool StockfishEngine::initialize(EngineInitResult& result) {
  logger_.infof("StockfishEngine: depth=%d, timeout=%dms, retries=%d",
...
void StockfishEngine::requestMove(const std::string& fen) {
  auto* ctx = new (std::nothrow) TaskContext();
  if (!ctx) {
    logger_.error("StockfishEngine: failed to allocate task context");
...
bool StockfishEngine::checkResult(EngineResult& result) {
...
void StockfishEngine::taskFunction(void* param) {
