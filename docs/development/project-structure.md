...
| `engines/librechess/assistance.h/.cpp` | `LibreChessAssistanceProvider`: implements `BoardAssistanceProvider` for BEST_MOVE assistance. Runs one 1 s `Game::rankCandidateTargets()` search over the lifted piece's legal targets inside a transient 64 KiB `lcAssist` task, falls back to `Game::scoreCandidateMove()` if needed, and returns `BoardMoveTargetRanking` best/worst destination data. |
