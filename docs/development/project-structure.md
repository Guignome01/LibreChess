...
| `trace.h/.cpp` | `eval` namespace: trace extraction for offline tuning. `TraceEntry` (param index + coefficient), `Trace` (entry list + helpers), `TrainingPosition` (trace + game result). `extractTrace(bb)` mirrors `evaluatePosition()` recording per-parameter linear contributions. `buildParamMap()` / `findParam()` for name↔index lookup. Moved from `lib/core/src/` — compiled only by the tuner Makefile (no `#ifdef TUNING` guard needed). |
| `Makefile` | Builds the tuner using g++ with `-DTUNING -std=gnu++17`. Compiles all core sources + local trace into object files, links with `-pthread`. Targets: `tune` (build), `pipeline` (build + run), `clean`. |
