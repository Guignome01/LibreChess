...
| File | Purpose |
|------|---------|
| `main.cpp` | Entry point: `setup()` and `loop()`. Game mode selection, WiFi/resign/board-edit relay, and game lifecycle management. Owns the shared `Board` plus long-lived board modes (`BoardGameplay`, `BoardDiagnostics`, `BoardCalibration`). |
| `board/board.h/.cpp` | Public physical-board entry point. Owns private `BoardSystem` + `BoardGui`, exposes a narrow public API (lifecycle, LED settings, sensor cadence), and grants a bounded friend list (`BoardGameplay`, `BoardDiagnostics`, `BoardCalibration`, `BoardMenu`, `BoardStatus`) access to internals via `Board::services()`. |
| `board/services.h/.cpp` | Single internal `BoardServices` facade returned by `Board::services()`. Forwards sensors/animations/LED settings; exposes the visual modules (`layering()/feedback()/assistance()/stack()`) and `makeCalibrationWorkflow()`. The only handle workflow classes use to reach board internals. |
| `board/menu.h/.cpp` | External `BoardMenu` workflow. Owns the game-selection menu flow (game/difficulty/color) plus folded-in `confirmAction()`/`confirmResume()` modal prompts rendered through transient `MenuView` instances. |
| `board/status.h/.cpp` | External `BoardStatus` workflow exposing low-level visual primitives needed by app/WiFi: `clearAllLEDs()` and the synchronous WiFi-connecting animation. |
| `board/gameplay_snapshot.h` | `OccupancySnapshot` helper: current/previous/changed occupancy state with template `sync()`/`update()` taking a `bool(int row, int col)` sensor-read callable. Used by `BoardGameplay`. |
| `board/config.h/.cpp` | Board-level static configuration. Currently holds menu item IDs/layouts and `configureMenus()` for wiring `MenuView` instances. |
| `board/gameplay.h/.cpp` | Board-owned physical chess interaction mode. Holds a `BoardServices&` and an `OccupancySnapshot`; detects lifts/placements/captures/resign gestures, drives setup/remote/castling guidance and gameplay feedback, and never mutates chess state. |
| `board/calibration.h/.cpp` | External `BoardCalibration` mode plus internal `BoardCalibrationWorkflow`. The workflow owns serial-guided calibration and NVS mapping persistence (`boardCal` namespace), using explicit `BoardDriver` calibration primitives for mapping tables that the driver applies during normal operation. |
| `board/diagnostics.h/.cpp` | External board diagnostics mode. The Sensor Test scans current debounced occupancy through `BoardServices`, records visited squares in its own state, lights them white via `BoardLayering`, and completes when all 64 are visited. |
| `board/core/system.h/.cpp` | Board-internal hardware/settings/scheduler service layer. Owns `BoardDriver` and `BoardScheduler`; batches direct LED writes through `BoardLEDBatch`; forwards current occupancy, generic animation submission, settings; and constructs a `BoardCalibrationWorkflow` on demand via `makeCalibrationWorkflow()`. |
| `board/core/colors.h` | `LedRGB` struct, named color constants (Cyan, White, Red, Green, Yellow, Purple, Orange, Blue, etc.), chess-side color mapping, and `scaleColor()` brightness helper. |
| `board/core/scheduler.h/.cpp` | Async board visual scheduler: FreeRTOS queue/task, LED mutex, stop flags, completion semaphore, and queue-drain barrier. |
...
| `board/gui/gui.h/.cpp` | Board-internal coordinator. Owns `BoardLayering`, `BoardFeedback`, `BoardAssistance`, `BoardStack`, and the `BoardServices` facade exposed back to `Board`. No forwarder methods. |
| `board/gui/layering.h/.cpp` | Board-internal LED composition layer. Stores persistent base visuals plus an overlay layer, exposes `BoardLayerWriter`, and renders composed frames through `BoardSystem::batchLEDs()`. |
| `board/gui/animations.h/.cpp` | Value-based `AnimationJob` definitions, factory helpers, animation metadata, and visual animation execution through `BoardLEDBatch`. `board/core/scheduler.*` owns queueing/concurrency; this module owns animation visuals. |
| `board/gui/selection.h` | Shared public-safe game-selection result types (`BoardGameSelection`, `BoardGameSelectionMode`) used by `BoardMenu` without exposing GUI internals. |
| `board/gui/drawable.h` | Minimal board-internal drawable contract for stackable visual surfaces. |
| `board/gui/assistance.h/.cpp` | Physical chess guidance: board setup prompts, configurable legal-move assistance, castling/remote move completion, and capture placement prompts. Constructed with mandatory `BoardSystem&` + `BoardLayering&`; all persistent visuals route through layering. Reads `Game` only through public APIs and never mutates chess state. |
| `board/gui/feedback.h/.cpp` | Visual feedback/status layer: illegal move blink, resign progress, move-result animations, check/game-end effects, thinking/waiting animations, remote game-end display, and error flashes. Constructed with mandatory `BoardSystem&` + `BoardLayering&`; no dual LED routes. |
| `board/gui/menu.h/.cpp` | Board-internal reusable `MenuView` `BoardDrawable` primitive. Displays options as colored LEDs, owns its empty-then-occupied selection debounce and orientation transform, supports orientation flipping, back buttons, and blink feedback. Constructed with mandatory `BoardSystem&` + `BoardLayering&`. The `BoardMenu` workflow constructs transient `MenuView` instances for confirm prompts. |
| `board/gui/stack.h/.cpp` | Board-internal modal visual stack (max depth 4). Push/pop navigation over non-owning `BoardDrawable*` entries, auto back-button handling, and parent drawable re-display. |

### Game Modes

| File | Purpose |
|------|---------|
| `game_mode/game_mode.h/.cpp` | Abstract base class for all game modes. Holds a `Game*` that orchestrates chess state, recording, and observer notification. Implements shared logic: `applyMove()` (the only `Game::makeMove()` call path for modes), coordinate-string parsing, `waitForBoardSetup()`, `tryResumeGame()`, web resign relay, and mode-specific resign hooks. Physical interaction is delegated to `BoardGameplay`; chess mutations stay in `GameMode`/`Game`. |
| `game_mode/bot_mode.h/.cpp` | Concrete class for human-vs-engine play (composition pattern). Composes an `EngineProvider*` via strategy injection. Non-blocking `update()` drives an async state machine (`BotState::PLAYER_TURN` / `ENGINE_THINKING`): player turn → `tryPlayerMove()` → `applyMove()` → `provider_->onPlayerMoveApplied()`; engine turn → `provider_->requestMove()` (spawns FreeRTOS task) → polls `provider_->checkResult()` each tick. BotMode owns engine lifecycle, thinking/waiting indicators, remote game-end handling, errors, and provider resign hooks; shared physical guidance lives in `BoardGameplay`. |
