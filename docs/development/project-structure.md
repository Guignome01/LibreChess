...
| `board/core/runtime.h/.cpp` | Board-internal hardware/canvas/effects/IO boundary. Owns `BoardDriver`, `BoardCanvas`, `BoardInput`, `BoardEffects`, and `BoardRenderer`; exposes `lockCanvas()` for canvas/effect mutation and synchronized input helpers (`drainInputEvents()`, `copyInputOccupancy()`, `inputOccupied()`); runs startup calibration through private `BoardCalibration` entry points. |
...
| `board/core/canvas.h/.cpp` | 7-layer pixel buffer (BACKGROUND, GAME, ASSISTANCE, FEEDBACK, MENU, EFFECT, OVERRIDE). Per-layer `LedRGB` plus `uint64_t` presence masks; `resolve(r, c)` returns the topmost present layer's colour; includes rect/fill/line/ring drawing helpers; `dirty()` flag picked up by the render task. |
| `board/core/effects.h/.cpp` | Slot-based animation manager (6 slots, `BoardEffectHandle{slot, generation}`). One-shot helpers (`startBlink`, `startFlash`, `startCapture`, `startPromotion`, `startFirework`) and looping helpers (`startThinking`, `startWaiting`, `startConnecting`); generation counter guards against ABA on slot reuse; full-layer effects reuse a retained scratch canvas for composition. |
| `board/core/input.h/.cpp` | Pure occupancy snapshot + 16-slot event ring (`LIFTED`/`PLACED`/`BASELINE_SYNCED`). Fed by the `BoardRuntime` poll task; direct access is not thread-safe, so workflows use `BoardRuntime` input helpers. |
| `board/core/renderer.h/.cpp` | FreeRTOS render task on Core 1 (~30 Hz, 4 KiB stack). Advances every active effect slot, picks up the canvas dirty flag, and flushes the resolved frame through `BoardDriver`. |
...
| `board/gui/layers.h` | `BoardLayer` enum identifying canvas layers in render order. |
| `board/gui/feedback.h/.cpp` | Visual feedback/status helper used by `BoardGameplay`. Owns illegal move blink, resign progress, move-result animations, check/game-end effects, thinking/waiting/error animations. All persistent visuals route through `BoardLayer::FEEDBACK` via the canvas guard; status animations return `BoardEffectHandle` for cancellation. |
| `board/gui/assistance.h/.cpp` | Physical chess guidance helper used by `BoardGameplay`. Owns board setup prompts, configurable legal-move assistance, castling/remote move completion, and capture placement prompts on `BoardLayer::ASSISTANCE`. Reads `Game` only through public APIs and never mutates chess state. |
| `board/gui/effect_animations.h/.cpp` | Pure step functions in the `BoardEffectSteps` namespace called by `BoardEffects` each render tick. Native-testable without Arduino/FreeRTOS. |
| `board/gui/selection.h` | Public-safe game-selection result types (`BoardGameSelection`, `BoardGameSelectionMode`) returned by `BoardMenu`. |
| `board/menus/view.h/.cpp` | `MenuView` reusable 8×8 LED menu primitive: paints to `BoardLayer::MENU` through `runtime.lockCanvas()`, snapshots occupancy once per poll for selection debounce, handles back button, orientation flipping, and the shared green/red confirmation prompt. |
...
| `board/workflows/diagnostics.h/.cpp` | Sensor Test workflow. Copies current debounced occupancy through `BoardRuntime`, records visited squares, lights them white via `BoardLayer::GAME`, and completes when all 64 are visited. |
