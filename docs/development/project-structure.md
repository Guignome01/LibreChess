...
| `src/piece.h` | `piece` namespace (header-only): type-safe piece representation (Piece/Color/PieceType enums, bit extraction, construction, predicates, color helpers, FEN char conversion via switches, `pieceZobristIndex` via `ZOBRIST_IDX_TABLE[16]` lookup with `ZOBRIST_IDX_NONE` sentinel and `isValidZobristIndex()` predicate, material values). |
