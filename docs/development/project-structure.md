...
| File | Purpose |
|------|---------|
| `main.cpp` | Entry point: `setup()` and `loop()`. Game mode selection, WiFi/resign/board-edit relay, and game lifecycle management. Owns the shared public `Board` plus a typed `GameSelectionMenu`; menu/diagnostics/calibration calls go through narrow `Board` facade methods, and game modes receive `BoardGameplay&` through `physicalBoard.gameplay()`. |
| `board/board.h/.cpp` | Public physical-board package root. Owns one internal `BoardRuntime`, one GUI `BoardAnimations` instance, a `BoardMenuRunner`, long-lived `BoardGameplay`/`BoardDiagnostics`, and the active `BoardAssistanceProvider`; exposes a narrow public API (`begin()` lifecycle result, LED settings, sensor cadence, `clearAllSurfaces()`, connecting status helpers, assistance provider installation, typed menu facade, diagnostics/calibration facades, workflow accessors); and keeps raw board internals private. |
| `board/core/runtime.h/.cpp` | Board-internal hardware/canvas/scheduler/IO boundary. Owns `BoardDriver`, `BoardCanvas`, `BoardInput`, `BoardScheduler`, and `BoardRenderer`; exposes `lockCanvas()` for synchronized canvas mutation, board-owned GUI adapter access to the scheduler/canvas, and synchronized input helpers (`drainInputEvents()`, `copyInputOccupancy()`, `inputOccupied()`); runs startup calibration through `BoardCalibrationRunner`; and shuts down renderer/input tasks with bounded cooperative waits. |
| `board/core/calibration.h/.cpp` | `BoardCalibrationRunner`: startup `load()`/`run()`/`save()` flow over raw `BoardDriver` access before renderer startup. Owns serial-guided calibration and NVS mapping persistence (`boardCal` namespace). |
| `board/core/driver.h/.cpp` | Hardware abstraction: LED strip (NeoPixelBus, I2S DMA), sensor grid (shift register scan + GPIO reads), saved calibration mapping application, LED settings (brightness, dimming), and GPIO pin definitions. |
| `board/core/canvas.h/.cpp` | Fixed-size ordered surface stack. Each surface stores `LedRGB[8][8]` plus a `uint64_t` presence mask; `resolve(r, c)` returns the newest present surface colour; includes rect/fill/line/ring drawing helpers; `dirty()` flag picked up by the render task. |
| `board/core/scheduler.h/.cpp` | Generic fixed-slot timed painter runner (6 slots, `BoardScheduledHandle{slot, generation}`) and `BoardPainter` callback contract. Owns allocation, cancellation, expiration, looping, fixed copied context storage, and one canvas surface per scheduled painter; generation counter guards against ABA on slot reuse. |
| `board/core/input.h/.cpp` | Pure occupancy snapshot + 16-slot event ring (`LIFTED`/`PLACED`/`BASELINE_SYNCED`) with dropped-event count and max-depth diagnostics. Fed by the `BoardRuntime` poll task; direct access is not thread-safe, so workflows use `BoardRuntime` input helpers. |
| `board/core/renderer.h/.cpp` | FreeRTOS render task on Core 1 (~30 Hz, 4 KiB stack). Runs active scheduled painters, picks up the canvas dirty flag, flushes the resolved frame through `BoardDriver`, and stops through a bounded graceful wait. |
| `board/core/helpers.h/.cpp` | Shared logical board dimensions and coordinate helpers (`BoardHelpers::ROWS`, `COLS`, `SQUARES`, `LAST_ROW`, `LAST_COL`, `inBounds()`) plus `BoardSurface`, the retained-surface helper for visual owners. Used by board DTOs, canvas, input, menus, diagnostics, visual helpers, and tests. |
| `board/core/menu/types.h` | Generic menu data: `MenuOption`, no-selection/back sentinels, and fixed option-count constants. |
| `board/core/menu/panel.h/.cpp` | `MenuPanel` shared physical-board menu primitive: owns one canvas surface, paints options through `runtime.lockCanvas()`, snapshots occupancy once per poll, debounces empty-then-occupied piece placement, handles orientation flipping, blinks confirmed selections through the injected `BoardAnimations&`, and returns option ids without interpreting them. |
| `board/core/menu/selection.h/.cpp` | `MenuSelection` selectable page wrapper over `MenuPanel`; copies one page of options into fixed storage and optionally appends the standard white back button. |
| `board/core/menu/menu.h/.cpp` | Typed menu contract and runner: `BoardMenu`, `BoardMenuController`, and `BoardMenuRunner`. The runner owns polling/drawing/debounce/blocking cadence loops while menu objects own semantic transitions and results. |
| `board/core/colors.h` | Self-contained `LedRGB` struct, named color constants (Cyan, White, Red, Green, Yellow, Purple, Orange, Blue, etc.), and `scaleColor()` brightness helper. Does not include chess/game headers. |
| `board/core/visual/feedback.h/.cpp` | Visual feedback/status helper used by `BoardGameplay`. Owns an explicit canvas surface for resign progress and starts illegal move blink, mapped move-result, check/game-end, thinking/waiting/error animations. Status animations return `BoardAnimationHandle` for cancellation. Consumes board-owned feedback DTOs only. |
| `board/core/visual/assistance.h/.cpp` | Physical chess guidance helper used by `BoardGameplay`. Owns an explicit canvas surface for board setup prompts, configurable legal-move/best-move assistance, castling/remote move completion, and capture placement prompts. Consumes board-owned setup/target/hint DTOs only; never imports `Game` or engine providers. |
| `board/core/visual/animations.h/.cpp` | Board-owned visual animation vocabulary, helper API, timing constants, status handle alias, and frame painters. Converts animation requests into `BoardPainter` callbacks scheduled by `BoardScheduler`; native-testable without Arduino/FreeRTOS. |
| `board/menus/game_selection.h/.cpp` | `GameSelectionMenu`: predefined game-selection tree (root/difficulty/color), option ids, `BoardGameSelection`, and `BoardGameSelectionMode`. Passed to `Board::showMenu()` at runtime. |
| `board/menus/confirm.h/.cpp` | `ConfirmMenu` and `ResumeConfirmMenu`: predefined green/red prompts used by resume and resign confirmation. Passed to `Board::runMenuBlocking()` at runtime. |
| `board/types.h` | Engine-agnostic board gameplay DTOs (legal target lists, setup snapshots, move feedback/completion data, best-move hints). These are the data shapes that keep `src/board/` independent from `Game`, `MoveList`, and concrete engines. |
| `board/game_provider.h` | Board-owned game provider contract (`BoardGameProvider`) consumed by `BoardGameplay` for setup and physical move validation. Concrete adapters live outside `src/board/`. |
| `board/assistance_provider.h` | Board-owned assistance provider contract plus fixed NONE and LEGAL_MOVES providers. Engine-backed BEST_MOVE providers live outside `src/board/` and return `BoardBestMoveHint` data only. |
| `board/workflows/gameplay.h/.cpp` | Long-lived physical chess interaction workflow. Takes `BoardRuntime&`, `BoardAnimations&`, and `BoardMenuRunner&`; holds `BoardFeedback` + `BoardAssistance`; drains synchronized `BoardInputEventBatch` values for player intent, detects lifts/placements/captures/resign gestures, validates against mapped legal targets, services BEST_MOVE assistance callbacks, runs resign confirmation through a typed `ConfirmMenu`, and drives setup/remote/castling guidance. Never mutates chess state. |
| `board/workflows/diagnostics.h/.cpp` | Sensor Test workflow. Copies current debounced occupancy through `BoardRuntime`, records visited squares, lights them white on an owned surface, and completes with an injected `BoardAnimations&` firework when all 64 are visited. |

### Game Modes

| File | Purpose |
|------|---------|
| `game_mode/board_adapter.h/.cpp` | Maps `LibreChess::Game`, `MoveList`, `MoveResult`, `CastlingInfo`, and core piece/color types into board-owned DTOs and the `BoardGameProvider` contract. This is the only GameMode-side mapping boundary for board workflows. |
| `game_mode/game_mode.h/.cpp` | Abstract base class for all game modes. Holds a `Game*` that orchestrates chess state, recording, and observer notification. Implements shared logic: `applyMove()` (the only `Game::makeMove()` call path for modes), coordinate-string parsing, `waitForBoardSetup()`, assistance service/cancel through board gameplay, `tryResumeGame()`, web resign relay, and mode-specific resign hooks. Physical interaction is delegated to `BoardGameplay`; chess mutations stay in `GameMode`/`Game`. |
| `game_mode/bot_mode.h/.cpp` | Concrete class for human-vs-engine play (composition pattern). Composes an opponent `EngineProvider*` via strategy injection. Non-blocking `update()` drives an async state machine (`BotState::PLAYER_TURN` / `ENGINE_THINKING`): player turn → service independent assistance → `tryPlayerMove()` → `applyMove()` → `provider_->onPlayerMoveApplied()`; engine turn → `provider_->requestMove()` (spawns FreeRTOS task) → polls `provider_->checkResult()` each tick. BotMode owns opponent engine lifecycle, thinking/waiting indicators, remote game-end handling, errors, and provider resign hooks; shared physical guidance lives in `BoardGameplay`. |
...
| `engines/librechess/assistance.h/.cpp` | `LibreChessAssistanceProvider`: implements `BoardAssistanceProvider` for BEST_MOVE assistance. Wraps an independent `LibreChessEngine`, lazily initializes/reuses `Game` search resources, discards stale results when the FEN changes, and maps coordinate moves to `BoardBestMoveHint`. |
...
| `src/provider.h` | Pure `EngineProvider` contract and data (`DifficultyLevel`, `EngineInitResult`, `EngineResult`). Shared by firmware game modes and concrete engines without pulling Arduino, FreeRTOS, board workflows, or firmware metadata into `lib/game`. `EngineInitResult::mode` is an opaque byte that firmware maps to `GameModeId`/`GameMeta`. |
...
| GPIO pin assignments | `src/board/core/driver.h` `#define`s | Edit source code |
