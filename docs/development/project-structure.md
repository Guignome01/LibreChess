...
| File | Purpose |
|------|---------|
| `main.cpp` | Entry point: `setup()` and `loop()`. Game mode selection, WiFi/resign/board-edit relay, and game lifecycle management. Owns the shared public `Board`; board workflows are owned by `Board` and accessed through `physicalBoard.gameplay()`, `.menu()`, `.diagnostics()`, and `.calibration()`. |
| `board/board.h/.cpp` | Public physical-board package root. Owns one internal `BoardRuntime`, one GUI `BoardAnimations` instance, plus long-lived `BoardGameplay`, `BoardDiagnostics`, `BoardCalibration`, and `BoardMenu` instances; exposes a narrow public API (`begin()` lifecycle result, LED settings, sensor cadence, `clearAllSurfaces()`, connecting status helpers, workflow accessors); and keeps raw board internals private. |
| `board/core/runtime.h/.cpp` | Board-internal hardware/canvas/scheduler/IO boundary. Owns `BoardDriver`, `BoardCanvas`, `BoardInput`, `BoardScheduler`, and `BoardRenderer`; exposes `lockCanvas()` for synchronized canvas mutation, presentation scheduler/canvas bindings for board-owned GUI adapters, and synchronized input helpers (`drainInputEvents()`, `copyInputOccupancy()`, `inputOccupied()`); runs startup calibration through private `BoardCalibration` entry points; and shuts down renderer/input tasks with bounded cooperative waits. |
...
| `board/gui/feedback.h/.cpp` | Visual feedback/status helper used by `BoardGameplay`. Owns an explicit canvas surface for resign progress and starts illegal move blink, move-result, check/game-end, thinking/waiting/error animations. Status animations return `BoardAnimationHandle` for cancellation. |
| `board/gui/assistance.h/.cpp` | Physical chess guidance helper used by `BoardGameplay`. Owns an explicit canvas surface for board setup prompts, configurable legal-move assistance, castling/remote move completion, and capture placement prompts. Reads `Game` only through public APIs and never mutates chess state. |
| `board/gui/animations.h/.cpp` | GUI-owned animation vocabulary, helper API, timing constants, and frame painters. Converts animation requests into `BoardPainter` callbacks scheduled by `BoardScheduler`; native-testable without Arduino/FreeRTOS. |
| `board/menus/options.h` | Public-safe game-selection result types (`BoardGameSelection`, `BoardGameSelectionMode`), result sentinels, menu option ids, and fixed option layouts for root/difficulty/color/confirmation menus. |
| `board/menus/panel.h/.cpp` | `MenuPanel` shared physical-board menu primitive: owns one canvas surface, paints options through `runtime.lockCanvas()`, snapshots occupancy once per poll, debounces empty-then-occupied piece placement, handles orientation flipping, blinks confirmed selections through the injected `BoardAnimations&`, and returns option ids without interpreting them. |
| `board/menus/selection.h/.cpp` | `MenuSelection` selectable screen wrapper over `MenuPanel`; copies one page of options into fixed storage and optionally appends the standard white back button. |
| `board/menus/prompt.h/.cpp` | `MenuPrompt` modal prompt helper built from `MenuPanel`; provides the green/red confirmation used by menu resume and resign confirmation. |
| `board/workflows/gameplay.h/.cpp` | Long-lived physical chess interaction workflow. Takes `BoardRuntime&` plus `BoardAnimations&`, holds `BoardFeedback` + `BoardAssistance`; drains synchronized `BoardInputEventBatch` values for player intent, detects lifts/placements/captures/resign gestures, and drives setup/remote/castling guidance. Never mutates chess state. |
| `board/workflows/menu.h/.cpp` | `BoardMenu` workflow: explicit `MenuStage` state machine over three `MenuSelection` screens (game/difficulty/color) and confirmation helpers that delegate to `MenuPrompt`. |
...
| `board/workflows/calibration.h/.cpp` | `BoardCalibration` workflow. External callers may call `trigger()`; `BoardRuntime` (friend) privately constructs it with `BoardDriver&` at startup to call `load()`/`run()`/`save()`. Owns serial-guided calibration and NVS mapping persistence (`boardCal` namespace). |
