...
| `board/board.h/.cpp` | Public physical-board package root. Owns one internal `BoardController` plus long-lived `BoardGameplay`, `BoardDiagnostics`, `BoardCalibration`, and `BoardMenu` instances; exposes a narrow public API (lifecycle, LED settings, sensor cadence, status helpers, workflow accessors); and keeps raw board internals private. |
| `board/workflow.h/.cpp` | Shared board workflow base. Stores the common `BoardController&` used by long-lived workflows so they share one hardware/visual runtime without depending on public `Board`. |
...
| `board/gameplay.h/.cpp` | Board-owned physical chess interaction mode. Inherits `BoardWorkflow`, holds a `BoardSnapshot<8, 8>`; detects lifts/placements/captures/resign gestures, drives setup/remote/castling guidance and gameplay feedback, and never mutates chess state. |
| `board/calibration.h/.cpp` | Unified `BoardCalibration` workflow. External callers may call `trigger()`; `BoardController` privately constructs it with `BoardDriver&` at startup to call `load()`/`run()`/`save()`. Owns serial-guided calibration and NVS mapping persistence (`boardCal` namespace), using explicit `BoardDriver` calibration primitives for mapping tables that the driver applies during normal operation. |
| `board/diagnostics.h/.cpp` | External board diagnostics mode. The Sensor Test scans current debounced occupancy through the shared `BoardController`, records visited squares in its own state, lights them white via `BoardLayering`, and completes when all 64 are visited. |
| `board/core/controller.h/.cpp` | Board-internal shared runtime owner and hardware/settings/scheduler boundary. Owns `BoardDriver`, `BoardScheduler`, `BoardLayering`, `BoardFeedback`, `BoardAssistance`, and `BoardStack`; batches direct LED writes through `BoardLEDBatch`; forwards current occupancy, animation submission, and LED settings; and runs startup calibration through private `BoardCalibration` entry points. |
| `board/core/snapshot.h` | Reusable fixed-grid occupancy transition helper. `BoardSnapshot<Rows, Cols>` stores current/previous/changed boolean occupancy state for board workflows that need physical edge detection. |
...
| `board/gui/layering.h/.cpp` | Board-internal LED composition layer. Stores persistent base visuals plus an overlay layer, exposes `BoardLayerWriter`, and renders composed frames through `BoardController::batchLEDs()`. |
...
| `board/gui/assistance.h/.cpp` | Physical chess guidance: board setup prompts, configurable legal-move assistance, castling/remote move completion, and capture placement prompts. Constructed with mandatory `BoardController&` + `BoardLayering&`; all persistent visuals route through layering. Reads `Game` only through public APIs and never mutates chess state. |
| `board/gui/feedback.h/.cpp` | Visual feedback/status layer: illegal move blink, resign progress, move-result animations, check/game-end effects, thinking/waiting animations, remote game-end display, and error flashes. Constructed with mandatory `BoardController&` + `BoardLayering&`; no dual LED routes. |
| `board/gui/menu.h/.cpp` | Board-internal reusable `MenuView` `BoardDrawable` primitive. Displays options as colored LEDs, owns its empty-then-occupied selection debounce and orientation transform, supports orientation flipping, back buttons, and blink feedback. Constructed with mandatory `BoardController&` + `BoardLayering&`. The `BoardMenu` workflow constructs transient `MenuView` instances for confirm prompts. |
