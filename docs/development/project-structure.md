...
| `board/services/menu/selection.h/.cpp` | `MenuSelection` shared physical-board menu primitive: owns one canvas surface, paints options through `runtime.lockCanvas()`, snapshots occupancy once per poll, debounces empty-then-occupied piece placement, handles orientation flipping, blinks confirmed selections through the injected `BoardAnimations&`, stores one selectable page plus optional white back button, and returns option ids without interpreting them. |
