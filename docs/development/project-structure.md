...
| `engines/librechess/engine.h/.cpp` | `LibreChessEngine`: extends `AsyncEngineProvider`, runs the on-board search engine in-process via `Game::calculateMove()`. Takes a `Game*` at construction. `initialize()` calls `game->initSearch()` with heap-sized TT + `game->setTimeFunc(millis)`, reserving heap for pawn/eval hashes, `SearchState`, minimum free heap, and the later 64 KiB `lcTask` stack before choosing TT size. Each `requestMove()` allocates its task context with `new(std::nothrow)`, spawns the FreeRTOS task, wires cancellation, calls `game->calculateMove(limits)`, and converts the `SearchResult` to `EngineResult`. No network, no string serialization. |
...
| File | Purpose |
|------|---------|
| `wifi_manager_esp32.h/.cpp` | WiFi connection management (state machine with AP/STA modes), async web server (ESPAsyncWebServer), all HTTP API endpoints, mDNS, known-networks registry (NVS), OTA password management, and board state relay to the web UI. Board state cache reads/writes are mutex-guarded because observer updates run on the main loop while `/board-update` is served from the async web task. |
