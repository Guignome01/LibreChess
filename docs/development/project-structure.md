...
| `board/board.h/.cpp` | Public physical-board package root. Owns one internal `BoardRuntime` plus long-lived `BoardGameplay`, `BoardDiagnostics`, `BoardCalibration`, and `BoardMenu` instances; exposes a narrow public API (`begin()` lifecycle result, LED settings, sensor cadence, `clearAllSurfaces()`, connecting status helpers, workflow accessors); and keeps raw board internals private. |
| `board/core/runtime.h/.cpp` | Board-internal hardware/canvas/scheduler/IO boundary. Owns `BoardDriver`, `BoardCanvas`, `BoardInput`, `BoardScheduler`, `BoardAnimations`, and `BoardRenderer`; exposes `lockCanvas()` for canvas/animation mutation and synchronized input helpers (`drainInputEvents()`, `copyInputOccupancy()`, `inputOccupied()`); runs startup calibration through private `BoardCalibration` entry points; and shuts down renderer/input tasks with bounded cooperative waits. |
...
| `board/menus/panel.h/.cpp` | `MenuPanel` shared physical-board menu primitive: owns one canvas surface, paints options through `runtime.lockCanvas()`, snapshots occupancy once per poll, debounces empty-then-occupied piece placement, handles orientation flipping, and returns option ids without interpreting them. |
...
| `board/workflows/diagnostics.h/.cpp` | Sensor Test workflow. Copies current debounced occupancy through `BoardRuntime`, records visited squares, lights them white on an owned surface, and completes when all 64 are visited. |
