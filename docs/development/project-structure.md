...
| `src/trace.h/.cpp` | `eval` namespace (`#ifdef TUNING` only): all tuning infrastructure consolidated into two files. **trace.h**: `TraceEntry`/`Trace`/`TrainingPosition` types, `extractTrace`/`buildParamMap`/`findParam` decls, descriptor structs (`ScalarParam`, `MobilityTableDef`, `PstDef`), `tuning::` accessor API decls, and all eval param `extern` declarations. **trace.cpp**: `ptrMap`/`nameMap`/`pIdx()` index maps, `extractTrace()` (mirrors `evaluatePosition()`), descriptor getter implementations (`scalarParams`, `mobilityDefs`, `pstDefs`), `buildRegistry()`, and `tuning::` accessor wrappers. Compiles to nothing in production builds. |
