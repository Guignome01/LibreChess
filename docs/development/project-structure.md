...
| File | Purpose |
|------|---------|
| `main.cpp` | Entry point: `setup()` and `loop()`. Game mode selection, WiFi/resign/board-edit relay, and game lifecycle management. Owns the shared public `Board` plus a typed `GameSelectionMenu`; menus run as overlays through the board facade, diagnostics starts with `Board::startProgram("diagnostics")`, and game modes receive the active `BoardGameProgram*` after `Board::startProgram("game")`. |
| `board/board.h/.cpp` | Public physical-board package root. Owns one internal `BoardRuntime`, board services (`BoardAnimations`, `BoardMenuRunner`, `BoardProgramRunner`), the persistent `BoardGameProgram` instance, a fixed program factory, and the active `BoardAssistanceProvider`; exposes a narrow public API (`begin()` lifecycle result, LED settings, sensor cadence, `update()`, `clearAllSurfaces()`, animation tokens, assistance provider installation, typed/named menu facade, dedicated `startGame()`/`stopGame()` returning the persistent `BoardGameProgram*`, and polled `startProgram(id)`/`stopProgram()` returning `BoardProgram*`); and keeps raw board internals and concrete programs private. |
...
| `board/services/menu/panel.h/.cpp` | `MenuPanel` shared physical-board menu primitive: owns one canvas surface, paints options through `runtime.lockCanvas()`, snapshots occupancy once per poll, debounces empty-then-occupied piece placement, handles orientation flipping, blinks confirmed selections through the injected `BoardAnimations&`, and returns option ids without interpreting them. |
| `board/services/menu/selection.h/.cpp` | `MenuSelection` selectable page wrapper over `MenuPanel`; copies one page of options into fixed storage and optionally appends the standard white back button. |
| `board/services/menu/menu.h/.cpp` | Typed menu contract and runner: `BoardMenu`, `BoardMenuController`, and `BoardMenuRunner`. The runner owns polling/drawing/debounce/blocking cadence loops while menu objects own semantic transitions and results. Typed menu instances are passed directly by callers (no factory layer). |
| `board/services/program/program.h/.cpp` | Polled board program contract and runner: `BoardProgram` plus `BoardProgramRunner`. The runner owns one active polled program slot, starts/cancels replacements, polls the active program, reports completion, and owns a factory-created `unique_ptr`. Menus remain independent overlays. The game program is permanent and lives on `Board::Impl` directly — it does not flow through this runner. |
| `board/services/program/factory.h/.cpp` | Fixed-size polled-program factory used by `Board` to create built-in polled programs (e.g. diagnostics) by stable string id. The persistent game program is not registered here — it is owned directly by `Board::Impl`. |
...
| `board/menus/game_selection.h/.cpp` | `GameSelectionMenu`: predefined game-selection tree (root/difficulty/color), option ids, `BoardGameSelection`, and `BoardGameSelectionMode`. Passed to `Board::showMenu()` when `main.cpp` needs typed result access. |
...
| `board/programs/game/game_rules.h` | Board-owned game rules contract (`BoardGameRules`) consumed by `BoardGameplay` for setup, lifted-piece lookup, and physical move validation. Concrete rules implementations live outside `src/board/`. |
...
| `board/programs/ids.h` | Stable string ids for primary board programs (`game`, `diagnostics`). |
| `board/programs/factory.h/.cpp` | Built-in program registration for `BoardProgramFactory`; this is the only board-level source that imports concrete gameplay/diagnostics headers. |
| `board/programs/game/game_program.h` | `BoardGameProgram` interface consumed by game modes. Keeps firmware mode code independent from the concrete `BoardGameplay` class. |
| `board/programs/game/gameplay.h/.cpp` | Board game physical-interaction program component. Implements `BoardGameProgram`, takes `BoardRuntime&`, `BoardAnimations&`, and `BoardMenuRunner&`; holds game-specific visuals (`BoardFeedback` + `BoardAssistance`); drains synchronized `BoardInputEventBatch` values for player intent, detects lifts/placements/captures/resign gestures, validates against mapped legal targets, services BEST_MOVE assistance callbacks, runs resign confirmation through a typed `ConfirmMenu`, and drives setup/remote/castling guidance. Never mutates chess state. |
...
| `board/programs/game/visuals/assistance.h/.cpp` | Game-program physical guidance visual helper. Owns a retained `BoardVisual` surface for board setup prompts, configurable legal-move/best-move assistance, castling/remote move completion, and capture placement prompts. Consumes board-owned setup/target/hint DTOs only; never imports `Game` or engine providers. |
| `board/programs/diagnostics/diagnostics_program.h/.cpp` | Sensor Test program. Implements `BoardProgram`, copies current debounced occupancy through `BoardRuntime`, records visited squares, lights them white on an owned `BoardVisual` surface, and completes with an injected `BoardAnimations&` firework when all 64 are visited. |

### Game Modes

| File | Purpose |
|------|---------|
| `game_mode/board_adapter.h/.cpp` | Maps `LibreChess::Game`, `MoveList`, `MoveResult`, `CastlingInfo`, and core piece/color types into board-owned DTOs and the `BoardGameRules` contract. This is the only GameMode-side mapping boundary for the board game program. |
