...
| `board/core/painter.h` | Generic logical-frame painter contract: copied context, paint mode, callback, and optional cleanup. Knows nothing about timing, hardware, or animation semantics. |
| `board/core/scheduler.h/.cpp` | Generic fixed-slot timed painter runner (6 slots, `BoardScheduledHandle{slot, generation}`). Owns allocation, cancellation, expiration, looping, and one canvas surface per scheduled painter; generation counter guards against ABA on slot reuse. |
...
| `board/menus/view.h/.cpp` | `MenuView` reusable 8×8 LED menu primitive: paints to its owned surface through `runtime.lockCanvas()`, snapshots occupancy once per poll for selection debounce, handles back button, orientation flipping, and the shared green/red confirmation prompt. |
| `board/menus/selection.h` | Public-safe game-selection result types (`BoardGameSelection`, `BoardGameSelectionMode`) returned by `BoardMenu`. |
...
| `board/workflows/menu.h/.cpp` | `BoardMenu` workflow: explicit `MenuStage` state machine over three `MenuView` instances (game/difficulty/color) plus folded-in `confirmAction(flipped)` and `confirmResume(mode, flipped)` modal prompts. |
