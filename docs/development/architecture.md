...
`LibreChessAssistanceProvider` under `src/engines/librechess/assistance.*` implements the board-owned `BoardAssistanceProvider` contract for BEST_MOVE assistance. It is installed on `Board`, separate from `BotMode`'s opponent provider, so assistance can be configured independently from the engine currently playing. On piece lift, `BoardGame` supplies the legal target list; the provider initializes/reuses Game-owned search resources and runs one synchronous 1 s root-filtered `Game::rankCandidateTargets()` search over only those destinations. It returns a `BoardMoveTargetRanking` for the best and worst lifted-piece destinations, falls back to `Game::scoreCandidateMove()` if search cannot produce scores, and never computes a global best move for the whole position.
...
`MenuSelection` (`src/board/services/menu/selection.*`) is the reusable physical-board primitive for the 8×8 LED grid. It takes a `BoardRuntime&` plus the board-owned `BoardAnimations&`, paints to an owned canvas surface via `runtime.lockCanvas()`, snapshots board occupancy once per poll, debounces empty-then-occupied placements, blinks confirmed selections through `BoardAnimations`, stores one selectable page plus optional white back button, and returns option ids without interpreting them. `BoardMenuRunner` (`src/board/services/menu/menu.*`) owns the active typed `BoardMenu`, the fixed page stack (`MENU_PAGE_STACK_DEPTH = 8`), polling, debounce, blocking cadence loops, and implements `MenuFlow` privately so it can apply queued transitions after each hook returns. State is fixed-size and stack/member allocated; no heap use in the runner.
...
Board assistance is configured separately from the opponent engine and persisted in the ESP32 NVS `"assist"` namespace by `/gameselect`. `NONE` and `LEGAL_MOVES` are fixed board providers. `BEST_MOVE` uses a board-owned `BoardAssistanceProvider`; the LibreChess-backed provider ranks the lifted piece's legal targets with a fixed 1 s root-filtered `Game::rankCandidateTargets()` search, so the green/red guidance is specific to the selected piece instead of a global best move for the full position. If search resources cannot be initialized, it falls back to the one-ply static `Game::scoreCandidateMove()` path.
