...
- `rook(sq, occ)`, `bishop(sq, occ)`, `queen(sq, occ)` — O(1) slider attacks via first-rank lookup table (rank) + Hyperbola Quintessence (file/diagonal/anti-diagonal). Diagonal masks (`DIAG[15]`, `ANTI_DIAG[15]`) indexed by diagonal number, precomputed at startup (~240B)
