...
- `pieceZobristIndex(piece)` — Zobrist hash table lookup via `ZOBRIST_IDX_TABLE[16]`; returns 0–11 for valid pieces or `ZOBRIST_IDX_NONE` (-1) sentinel; `isValidZobristIndex()` predicate for bounds checking
