...
`LibreChessAssistanceProvider` under `src/engines/librechess/assistance.*` implements the board-owned `BoardAssistanceProvider` contract for BEST_MOVE assistance. It is installed on `Board`, separate from `BotMode`'s opponent provider, so assistance can be configured independently from the engine currently playing. On piece lift, `BoardGame` supplies the legal target list, immediately paints neutral white legal destinations, then the provider initializes/reuses Game-owned search resources and runs one 1 s root-filtered `Game::rankCandidateTargets()` search over only those destinations inside a transient 64 KiB `lcAssist` FreeRTOS task, while the board gesture path waits synchronously for the result. It returns a `BoardMoveTargetRanking` for the best and worst lifted-piece destinations, falls back to `Game::scoreCandidateMove()` if search cannot produce scores, and never computes a global best move for the whole position.
