...
**Board state relay** — `WiFiManagerESP32` implements `IGameObserver`. `Game` calls `onBoardStateChanged(fen, evaluation)` automatically after every board mutation. The observer caches the FEN, evaluation, and navigation state (move index, move count, undo/redo availability, move list). The web UI polls `GET /board-update` which returns all cached state as JSON. The observer runs on the main loop task while `/board-update` runs on the async web task, so reads/writes of the cached board state are guarded by `boardStateMutex_` and copied into a local snapshot before JSON serialization.
