...
Core internals use **LERF (Little-Endian Rank-File)** square indexing natively: a1=0, b1=1, ..., h8=63. Primary conversions: `rankOf(sq) = sq / 8`, `fileOf(sq) = sq % 8`, `makeSquare(rank, file) = rank * 8 + file`. The game layer and firmware use **row/col** display coordinates (row 0 = rank 8, col 0 = file a). Boundary helpers (`squareOf(row, col)`, `rowOf(sq)`) in `bitboard.h` convert between the two systems. Board mailbox is a flat `[64]` LERF-indexed array; `INITIAL_BOARD[64]` holds the starting position.
