...
Engine (lib/engine/):
  ...
  search (alpha-beta + quiescence + iterative deepening + check ext + PVS + NMP + LMR + LMP + razoring + lazy eval + aspiration windows + IID + delta pruning + futility pruning + SEE-based ordering + countermove heuristic)
...
`eval` (in `lib/core/`, `evaluation.h/cpp`) is a namespace providing tapered position evaluation via `evaluatePosition(const BitboardSet& bb)`. Returns a score in centipawns (positive = White advantage). Computes separate midgame (MG) and endgame (EG) scores, each combining material counting (P=100, N=300, B=300, R=500, Q=900), phase-specific piece-square tables, pawn structure bonuses/penalties (passed, isolated, doubled, backward, connected passers via file-scoped pawn mask arrays initialized lazily by `initPawnMasks()`), and positional terms: bishop pair (+30 MG / +50 EG), rook on open file (+20) / semi-open file (+10), rook on 7th rank (+20 MG / +30 EG), piece mobility (per-square bonuses via `attacks::computeAll()` → `AttackInfo`), king safety / pawn shield (missing pawn −15, advanced pawn −5, open file −20, MG only), knight outposts (+15, doubled for central squares), center control (pawn occupation +20 / pawn attack +10 for d4/d5/e4/e5, MG+EG), king tropism (Chebyshev distance penalty for attacker proximity to enemy king, MG only), space (safe squares behind pawn chain in files c–f, MG+EG), trapped pieces (bishop trapped on a7/h7/a2/h2 by enemy pawn −50 MG, rook trapped by own uncastled king −40 MG), and connectivity (bonus per non-pawn non-king piece defended by a friendly piece, +3 MG/EG, uses `AttackInfo::byColor`). Tempo bonus (+10 cp for side to move) is applied in the search layer's `evaluate()` wrapper. King and pawn have distinct MG/EG PSTs (king: safety → centralization; pawn: center control → advancement); other pieces share PSTs across phases. Game phase is derived from non-pawn material (N=1, B=1, R=2, Q=4; max 24); final score interpolates: `(mg * phase + eg * (24 - phase)) / 24`. Based on the simplified evaluation function (CPW / Tomasz Michniewski), extended with positional heuristics. Black pieces mirror via `sq ^ 56`. Iterates piece bitboards via `popLsb` — no mailbox needed.
